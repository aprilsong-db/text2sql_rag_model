...
The databricks CLI bundles top level is defined by file `text2sql_rag_app/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`text2sql_rag_app/resources/model-workflow-resource.yml`)
 - batch inference workflow (`text2sql_rag_app/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`text2sql_rag_app/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`text2sql_rag_app/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`text2sql_rag_app/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `text2sql_rag_app/databricks.yml` and `text2sql_rag_app/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-text2sql_rag_app-model     | /dev-text2sql_rag_app-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest dev content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-text2sql_rag_app-model | /staging-text2sql_rag_app-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-text2sql_rag_app-model    | /prod-text2sql_rag_app-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the dev branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-text2sql_rag_app-model    | /test-text2sql_rag_app-experiment    |
...
2. Complete the `TODO` in `text2sql_rag_app/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `text2sql_rag_app/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `text2sql_rag_app/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-text2sql_rag_app-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-text2sql_rag_app-batch-inference-job`
that runs the notebook under `text2sql_rag_app/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-text2sql_rag_app-batch-inference-job`.
...
`test-text2sql_rag_app-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-text2sql_rag_app-batch-inference-job`.
